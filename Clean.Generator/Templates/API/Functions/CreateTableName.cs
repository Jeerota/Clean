...
namespace Clean.API.Functions.ContextNameContext.TableName.Functions
{
    public class CreateTableName
    {
        ...
        private readonly ILogger<CreateTableName> _logger;

        public CreateTableName(ILogger<CreateTableName> log, ITableNameService TableNameService)
        {
            _TableNameService = TableNameService;
            _logger = log;
        }

        [FunctionName("CreateTableName")]
        public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = "TableName")] HttpRequest req)
        {
